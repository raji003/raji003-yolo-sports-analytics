...
DATASET = Path("Images for training")  # from step1_copy_convert.py
SEED = 42
...
# Create data.yaml from notes.json (if available)
notes_path = DATASET / "notes.json"
data_yaml_path = DATASET / "data.yaml"

if not notes_path.exists():
    print("notes.json not found; data.yaml was not created.")
else:
    ...
    if not id_to_name:
        print("No categories found; data.yaml was not created.")
    else:
        max_id = max(id_to_name.keys())
        names = []
        for cid in range(max_id + 1):
            raw = id_to_name.get(cid, f"class_{cid}")
            safe = raw.replace("\"", "\\\"")
            names.append(f"\"{safe}\"")
        ...
        lines += [f"  {idx}: {name}" for idx, name in enumerate(names)]
